...
const AdminSettings = () => {
      ...
      try {
        ...
        if (active) {
          setSettings({
            ...
            message: data?.message || 'We are updating the store. Please check back soon.',
          });
        }
      } catch (err) {
  ...
  const saveSettings = async (nextSettings = settings) => {
    ...
    try {
      ...
      toast.success('Settings Saved', saved?.isOnline === false ? 'Website is now OFF.' : 'Website is now ON.');
    } catch (err) {
      setError(err?.message || 'Unable to save settings.');
      toast.error('Save Failed', err?.message || 'Unable to save settings.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex min-h-[360px] flex-col items-center justify-center gap-4">
        ...
        <div className="text-xs uppercase tracking-[0.3em] text-slate-400 font-medium">Loading settings...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">Turn the customer website on or off from one place.</p>
      </div>
